...
> **Potensi Issue:** Kondisi `$dpPaid === 0` menggunakan strict comparison. Jika tipe data dari `sum()` mengembalikan `"0"` (string) bukan `0` (integer), maka `=== 0` akan FALSE dan logika fallback tidak berjalan. Perlu diuji apakah `DB::sum()` di Laravel selalu mengembalikan integer atau bisa string/float.

**Tabel Kasus Uji Desk Checking:**

| ID | Skenario | Input | Expected Output | Status |
|----|----------|-------|----------------|--------|
| DC-01 | Belum ada DP | dpPaid = 0, total = 25.000.000 | Sisa = 25.000.000 | Pass |
| DC-02 | DP 30% sudah dibayar | dpPaid = 7.500.000, total = 25.000.000 | Sisa = 17.500.000 | Pass |
| DC-03 | DP melebihi total (edge case) | dpPaid = 26.000.000, total = 25.000.000 | Sisa = 0 (max protection) | Pass |
...
public function store(Request $request)
{
    $request->validate([
        ...
        // TEMUAN: Tidak ada validasi jumlah_tamu di sini!
    ]);
    ...
    $booking = Booking::create([
        ...
        // TEMUAN: jumlah_tamu digabung ke notes, bukan field tersendiri!
        'notes'        => $request->notes,
        'status'       => 'pending',
...
|----|----------|----------|----------|--------|
| CW-T01 | Bypass validasi tamu via Postman (kirim langsung ke API) | POST /api/bookings dengan tamu=1000 | Seharusnya ditolak, tapi sistem menerima | Fail |
| CW-T02 | Dua request booking tanggal sama secara bersamaan | Concurrent request dengan curl | Hanya 1 booking terbentuk | Fail |
...
|----|-------|---------|-------|----------|--------|--------|
| CF-01 | Jalur A (404) | User tidak ditemukan | email="tidakada@test.com" | HTTP 404, "Akun tidak ditemukan" | HTTP 404, "Akun tidak ditemukan" | Pass |
| CF-02 | Jalur B (401) | Password salah | email="ada@test.com", pass="salah" | HTTP 401, "Password salah" | HTTP 401, "Password salah" | Pass |
| CF-03 | Jalur C (403) | Akun nonaktif | email="nonaktif@test.com", pass="benar" | HTTP 403, "Akun dinonaktifkan" | HTTP 403, "Akun Anda dinonaktifkan" | Pass |
| CF-04 | Jalur D (200) | Login berhasil | email="aktif@test.com", pass="benar" | HTTP 200 + token JWT | HTTP 200 + token JWT | Pass |
...
| Nodes (N) | 8 |
| Predicate Nodes (P) | 1 |
| Cyclomatic Complexity V(G) = E - N + 2P | **3** |
...
|----|------|-------|----------|--------|--------|
| BP-01 | Path 1 | wedding_date yang sudah dibooking | HTTP 422: "Tanggal tersebut sudah dipesan" | HTTP 422: "Tanggal tersebut sudah dipesan" | Pass |
| BP-02 | Path 2 | Data valid, tanggal belum dibooking | HTTP 201: "Pemesanan berhasil" + data booking | HTTP 201: "Pemesanan berhasil" + data booking | Pass |
| BP-03 | Path 3 | package_id = 9999 (tidak ada) | HTTP 404 exception | HTTP 404 exception | Pass |
...
|----|----------|-------|----------|--------|--------|
| LOOP-01 | Loop normal 3 iterasi, harga custom | price_silver=20jt, gold=40jt, platinum=80jt | 3 Package terbentuk dengan harga sesuai | 3 Package terbentuk dengan harga sesuai | Pass |
| LOOP-02 | Nilai duplikat (silver=gold) | price_silver=40jt, price_gold=40jt, platinum=80jt | 3 Package terbentuk (duplikat harga valid) | 3 Package terbentuk (duplikat harga valid) | Pass |
| LOOP-03 | Semua nilai default (tanpa input price) | Tanpa price_silver/gold/platinum | silver=25jt, gold=45jt, platinum=85jt (default) | silver=25jt, gold=45jt, platinum=85jt (default) | Pass |
...
|--------|-----------|------|------|-------------|
| Desk Checking | 3 | 3 | 0 | 0 |
| Code Walkthrough | 2 | 0 | 2 | 0 |
| Control Flow Testing | 4 | 4 | 0 | 0 |
| Basic Path Testing | 3 | 3 | 0 | 0 |
| Loop Testing | 3 | 3 | 0 | 0 |
| **Total** | **15** | **13** | **2** | **0** |
