...
function WorkflowPanel({ booking, vendors, onUpdated }) {
  ...
  async function doExecute() {
    ...
    try {
      ...
      alert(err.userMessage || "Gagal eksekusi.");
    } finally {
      setActing(false);
    }
  }
  ...
  async function doMarkCompleted() {
    if (
      !window.confirm(
        "Tandai acara selesai dan pembayaran otomatis dianggap tuntas?",
      )
    )
      return;
    setActing(true);
    try {
      await adminService.markEventCompleted(booking.id);
      onUpdated();
    } catch (err) {
      alert(err.userMessage || "Gagal menandai acara selesai.");
    } finally {
      setActing(false);
    }
  }
  ...
  function getStepIndex(status) {
    ...
    if (
      ...
      return 1;
    if (status === "vendor_confirmed" || status === "tech_meeting_scheduled")
      return 2;
    ...
    <div className="mt-5 border-t border-[var(--color-cream-border)] pt-5 space-y-4">
      ...
      {(s === "waiting_dp" || s === "payment_failed") && (
        <div className="space-y-3 p-4 bg-[var(--color-cream)] border border-[var(--color-cream-border)]">
          ...
          <p className="text-xs text-[var(--color-slate)] font-[var(--font-sans)]">
            Klien belum membayar DP 30% atau pembayaran gagal. Anda dapat
            mengonfirmasi pembayaran DP secara manual jika klien telah
            mentransfer secara manual.
          </p>
          <Button
            size="sm"
            variant="gold"
            isLoading={acting}
            onClick={async () => {
              if (!window.confirm("Konfirmasi pembayaran DP secara manual?"))
                return;
              setActing(true);
              try {
                await adminService.confirmPayment(booking.id, {
                  payment_type: "dp30",
                });
                onUpdated();
              } catch (err) {
      ...
      {s === "in_event" && (
        <div className="space-y-3 p-4 bg-green-50 border border-green-200">
          <p className="text-xs font-medium text-green-800 font-[var(--font-sans)]">
            🎉 Acara Sedang Berlangsung
          </p>
          <p className="text-xs text-green-700 font-[var(--font-sans)]">
            Acara sedang berjalan. Setelah acara selesai, klik tombol di bawah
            untuk menandai acara selesai dan pembayaran akan otomatis dianggap
            tuntas.
          </p>
          <Button
            size="sm"
      ...
      {booking.tech_meeting_at && (
        <div className="p-2 bg-purple-50 border border-purple-100 text-[11px] rounded text-purple-900 font-[var(--font-sans)] leading-relaxed">
          <strong>📅 Jadwal Tech Meeting:</strong>{" "}
          {new Date(booking.tech_meeting_at).toLocaleString("id-ID", {
            dateStyle: "short",
            timeStyle: "short",
          })}
      ...
      {booking.vendor_requests?.length > 0 && (
        <div>
          ...
          {booking.vendor_requests.map((vr, i) => (
            <div
              key={i}
              className="py-1.5 border-b border-[var(--color-cream-border)] last:border-0 font-[var(--font-sans)]"
            >
              ...
              {vr.status === "confirmed" && vr.vendor_notes && (
                <div className="mt-1 p-2 bg-teal-50 border border-teal-100 rounded text-teal-800 text-[11px] leading-relaxed">
                  <strong>💬 Detail Pertemuan / Koordinasi:</strong>{" "}
                  {vr.vendor_notes}
                </div>
              )}
            </div>
...
export default function AdminBookings() {
  ...
  async function handleDeleteBooking(b) {
    if (
      !window.confirm(
        `Yakin ingin menghapus booking ${b.order_id} secara permanen? Tindakan ini akan menghapus pembayaran dan request vendor terkait.`,
      )
    )
      return;
    try {
  ...
  async function handleDeleteAllBookings() {
    if (
      !window.confirm(
        "APAKAH ANDA YAKIN? Tindakan ini akan menghapus SEMUA data booking dan pembayaran di sistem secara permanen!",
      )
    )
      return;
    try {
