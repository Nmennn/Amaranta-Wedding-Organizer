...
function WorkflowPanel({ booking, vendors, onUpdated }) {
  ...
  const [techForm, setTechForm] = useState({ tech_meeting_at: '', tech_meeting_location: '', tech_meeting_notes: '' })
  ...
  async function doAssign() {
    ...
    setActing(true)
    try {
      await adminService.assignVendor(booking.id, { vendor_id: Number(assignVendorId), admin_notes: adminNotes })
      onUpdated()
    } catch (err) { alert(err.userMessage || 'Gagal assign vendor') }
  ...
  async function doReassign() {
    ...
    setActing(true)
    try {
      await adminService.reassignVendor(booking.id, { vendor_id: Number(assignVendorId) })
      onUpdated()
    } catch (err) { alert(err.userMessage || 'Gagal reassign vendor') }
    finally { setActing(false) }
  }

  async function doTechMeeting() {
    if (!techForm.tech_meeting_at || !techForm.tech_meeting_location) {
      alert('Tanggal dan lokasi wajib diisi'); return
    }
    setActing(true)
    try {
      await adminService.setTechMeeting(booking.id, techForm)
      onUpdated()
    } catch (err) { alert(err.userMessage || 'Gagal atur tech meeting') }
    finally { setActing(false) }
  }

  async function doConfirmTech() {
    setActing(true)
    try { await adminService.confirmTech(booking.id); onUpdated() }
    catch (err) { alert(err.userMessage || 'Gagal') }
    finally { setActing(false) }
  }

  async function doUpdatePrep() {
    setActing(true)
    try { await adminService.updatePreparation(booking.id, prepPct); onUpdated() }
    catch (err) { alert(err.userMessage || 'Gagal') }
    finally { setActing(false) }
  }
  ...
  async function doExecute() {
    ...
    setActing(true)
    try { await adminService.executeEvent(booking.id); onUpdated() }
    catch (err) { alert(err.userMessage || 'Gagal eksekusi. Pastikan pembayaran sudah lunas.') }
    finally { setActing(false) }
  }
  ...
  return (
    <div className="mt-5 border-t border-[var(--color-cream-border)] pt-5 space-y-4">
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {STEPS.map((step, i) => (
          <div key={step} className="flex items-center gap-1">
            ...
            {i < STEPS.length - 1 && (
              <span className="text-[var(--color-cream-border)] text-xs">›</span>
            )}
          </div>
        ))}
      </div>
      ...
      {(s === 'waiting_vendor' || s === 'vendor_rejected') && (
        <div className="space-y-3 p-4 bg-[var(--color-cream)] border border-[var(--color-cream-border)]">
          <p className="text-xs font-medium text-[var(--color-dark)] font-[var(--font-sans)]">
            {s === 'vendor_rejected' ? '🔄 Pilih Vendor Pengganti' : '👤 Assign Vendor'}
          </p>
          <select value={assignVendorId} onChange={e => setAssignVendorId(e.target.value)}
            ...
            {vendors.filter(v => v.status === 'approved').map(v => (
              <option key={v.id} value={v.id}>{v.name} ({v.category || 'Umum'})</option>
            ))}
          </select>
          <Input label="Catatan untuk Vendor (opsional)" value={adminNotes}
            onChange={e => setAdminNotes(e.target.value)} />
          <Button size="sm" variant="gold" isLoading={acting} disabled={!assignVendorId}
      ...
      {s === 'vendor_confirmed' && (
        <div className="space-y-3 p-4 bg-[var(--color-cream)] border border-[var(--color-cream-border)]">
          <p className="text-xs font-medium text-[var(--color-dark)] font-[var(--font-sans)]">📅 Jadwalkan Tech Meeting</p>
          <Input label="Tanggal & Waktu" type="datetime-local" value={techForm.tech_meeting_at}
            onChange={e => setTechForm(p => ({ ...p, tech_meeting_at: e.target.value }))} />
          <Input label="Lokasi Meeting" value={techForm.tech_meeting_location}
            onChange={e => setTechForm(p => ({ ...p, tech_meeting_location: e.target.value }))}
            placeholder="Zoom / Kantor AMARANTA / ..." />
          <Input label="Catatan (opsional)" value={techForm.tech_meeting_notes}
            onChange={e => setTechForm(p => ({ ...p, tech_meeting_notes: e.target.value }))} />
          <Button size="sm" variant="gold" isLoading={acting} onClick={doTechMeeting}>Jadwalkan</Button>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 p-3 bg-purple-50 border border-purple-200">
          <div>
            <p className="text-xs font-medium text-purple-800 font-[var(--font-sans)]">
              Tech Meeting: {booking.tech_meeting_at ? new Date(booking.tech_meeting_at).toLocaleString('id-ID') : '—'}
            </p>
            <p className="text-xs text-purple-600 font-[var(--font-sans)]">
              Lokasi: {booking.tech_meeting_location || '—'}
            </p>
          </div>
          <Button size="sm" variant="gold" isLoading={acting} onClick={doConfirmTech}>
      ...
      {s === 'preparation' && (
        <div className="space-y-3 p-4 bg-[var(--color-cream)] border border-[var(--color-cream-border)]">
          ...
          <input type="range" min={0} max={100} step={5} value={prepPct}
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" isLoading={acting} onClick={doUpdatePrep}>
              Simpan Progress
            </Button>
            <Button size="sm" variant="primary" isLoading={acting} onClick={doExecute}>
              🚀 Eksekusi Acara
            </Button>
          </div>
        </div>
...
export default function AdminBookings() {
  ...
  const needAction = bookings.filter(b =>
    ['waiting_vendor', 'vendor_rejected', 'vendor_confirmed'].includes(b.admin_status)
  ).length

  const filtered = bookings
    .filter(b => filter === 'all' || b.admin_status === filter)
    .filter(b => !search || b.order_id?.includes(search) ||
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="font-[var(--font-display)] text-3xl text-[var(--color-dark)] mb-1">Manajemen Booking</h1>
        <p className="text-sm text-[var(--color-slate)] font-[var(--font-sans)]">
          Kelola booking sebagai Wedding Organizer — assign vendor, tech meeting, persiapan.
        </p>
      </div>
